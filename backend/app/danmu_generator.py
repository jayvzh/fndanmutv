...
class DanmuAPI:
    ...
    TIMEOUT = (10, 60)
    # 默认 21s ≈ 3 次/分钟（danmu-api 新版默认限流 RATE_LIMIT_MAX_REQUESTS=3），
    # 可通过配置 api_request_interval 运行时调整
    _MIN_REQUEST_INTERVAL = 21.0
    ...
    @classmethod
    def set_request_interval(cls, seconds: float):
        """设置最小请求间隔（秒），0 或负数表示不节流"""
        try:
            value = float(seconds)
        except (TypeError, ValueError):
            value = cls._MIN_REQUEST_INTERVAL
        cls._MIN_REQUEST_INTERVAL = max(0.0, value)
    ...
    @classmethod
    def _rate_limit_wait(cls, response, retry: int) -> float:
        """计算 429 后的等待秒数：优先响应 Retry-After 头，否则本地指数退避。

        本地退避基线取 max(30s, 当前节流间隔)：上游限流窗口为 1 分钟，
        旧的 3/6/12/24s 阶梯总等待不足一个窗口，重试大概率仍 429。
        """
        try:
            ra = response.headers.get("Retry-After") if response is not None else None
            if ra:
                return min(max(float(ra), 1.0), 3600.0)
        except (ValueError, TypeError):
            pass
        return max(30.0, cls._MIN_REQUEST_INTERVAL) * (2 ** retry)
    ...
    def get_comment_id(cls, file_path: str, use_tmdb_id: bool = False, tmdb_id: Optional[int] = None, episode: Optional[int] = None, cache_ttl: Optional[int] = None, tmdb_id_type: int = 0) -> Optional[str]:
        ...
        try:
            ...
            # 有集数（剧集）使用 "标题.S01E集数"；无集数（电影）直接用标题
            ...
            logger.info(f"使用匹配文件名: {match_file_name}")
            ...
            for retry in range(4):
                ...
                if response.status_code == 200:
                    ...
                    wait = cls._rate_limit_wait(response, retry)
                    logger.warning(f"匹配请求被限流(429)，等待{wait:.0f}秒后重试 ({retry+1}/4)")
    ...
    def _extract_title_from_filename(file_name: str) -> Optional[str]:
        ...
        - 循环去除多重扩展名（如 .mkv.strm）
        - 保留年份（如 (2010)），年份对识别很重要
        - 移除分辨率/编码/来源等质量标记（1080P、x265、HEVC、BluRay、WEB-DL、HDR 等）
        - 移除集数标识（S01E01、E01 等）
        """
        name = file_name
        # 循环去除所有已知扩展名（处理 .mkv.strm 这类多重后缀）
        while True:
            base, ext = os.path.splitext(name)
            if ext.lower() in ('.strm', '.mp4', '.mkv', '.avi', '.rmvb', '.wmv',
                               '.mov', '.m4v', '.flv', '.ts', '.m2ts', '.iso'):
                name = base
            else:
                break
        ...
        # 移除集数标识（S01E01 / E01，单独成行或边界）
        name = re.sub(r'[sS]\d+[eE]\d+', ' ', name)
        name = re.sub(r'(?<![A-Za-z0-9])[eE]\d+(?![A-Za-z0-9])', ' ', name)

        # 移除分辨率标记：1080P / 720P / 2160P / 4K / 8K / 480i 等
        name = re.sub(r'\b(?:2160|1080|720|576|480)\s*[piPI]\b', ' ', name)
        name = re.sub(r'\b[48][kK]\b', ' ', name)

        # 移除编码/来源/质量标记
        quality_tokens = [
            r'[xXhH]\.?26[45]', r'[xX]26[45]', r'[hH]evc', r'[aAvV][cC]1?',
            r'[bB]lu[rR]ay', r'[bB][rR][rR]ip', r'[wW][eE][bB][-\s]?[dD][lL]',
            r'[wW][eE][bB][rR]ip', r'[hH][dD][rR](?:10)?', r'10\s*bit', r'8\s*bit',
            r'[rR]e[mM][uU][xX]', r'[dD]ts', r'[dD]olby', r'[tT]rue[dD]5?',
            r'[aA][aA][cC]2?', r'[fF][lL][aA][cC]', r'[dD][dD][pP]?5?\.?1',
            r'[aA][tT][mM][oO][sS]', r'[nN][fF]', r'[aA][mM][zZ][oO][nN]',
            r'[dD][iI][sS][nN][eE][yY]?\+?', r'[bB][iI][lL][iI][bB][iI][lL][iI]',
        ]
        ...
        # 移除年份之后的所有内容（年份保留；例如 "盗梦空间 (2010) 1080P ..." -> "盗梦空间 (2010)"）
        ...
        try:
            ...
            for retry in range(4):
                ...
                if response.status_code == 200:
                    ...
                    wait = cls._rate_limit_wait(response, retry)
                    logger.warning(f"获取弹幕被限流(429)，等待{wait:.0f}秒后重试 ({retry+1}/4)")
                    cls._trigger_rate_limit(wait)
                    time.sleep(wait)
                else:
                    last_error = "network"
            ...
            if response and response.status_code == 200:
                ...
                return result, ""
            logger.error(f"获取弹幕失败: {response.text if response is not None else '无响应'}")
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            for comment in comments:
                try:
                    ...
                    if pos not in (4, 5):  # 非顶/底弹幕均视为滚动弹幕（mode 0/1/2/3/6）
